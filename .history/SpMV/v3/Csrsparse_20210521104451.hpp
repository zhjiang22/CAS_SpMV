...
template <typename T, >
__global__ void spmv_light_kernel(, int* d_ptr, int*
    colindex,T* value, T* x, T*
    y,int N) {
}
                  ...
                  double* hy)
{
	...
	cudaMalloc()
    device_sparse_spmv<<<grid_dim, block_dim, block_dim * sizeof(double)>>>(htrans,halpha,hbeta,hm,hn,hrowptr,hcolindex,hvalue,hx,hy, 32, 1024 / 32, cudaRowCounter);
}