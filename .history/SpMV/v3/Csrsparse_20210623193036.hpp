...
template <int int THREADS_PER_VECTOR>
                     			   ...
                     			   double* y)
{
	...
	__shared__ volatile int ptrs[blockDim.x / VECTORS_PER_BLOCK][2];
  	// Get the row index
...
double* hy)
{
	...
	if (mean_elements <= 2) {
		const int temp = 2;
    	device_sparse_spmv<temp><<<grid_dim, block_dim>>>(htrans,halpha,hbeta,hm,hn,hrowptr,hcolindex,hvalue,hx,hy, cudaRowCounter);
	}
