                  ...
                  double* hy)
{
	...
	int sum_k = NonZ + hm;
	int grid_dim = (sum_k + (block_dim * 6400) - 1) / (block_dim * 6400);
	printf("sum_k: %d grid: %d NonZ: %d\n", sum_k, grid_dim, NonZ);
    device_sparse_spmv<<<grid_dim, block_dim, block_dim * sizeof(double)>>>(htrans,halpha,hbeta,hm,hn,NonZ,6400, hrowptr,hcolindex,hvalue,hx,hy);
}