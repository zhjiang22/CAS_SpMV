...
double* hy)
{
	...
	if (mean_elements <= 2) {
		const int temp = 2;
    	device_sparse_spmv<temp><<<grid_dim, block_dim, block_dim * sizeof(double)>>>(htrans,halpha,hbeta,hm,hn,hrowptr,hcolindex,hvalue,hx,hy, cudaRowCounter);
	}
	...
	const int p = 8;
    device_sparse_spmv<p><<<grid_dim, block_dim, block_dim * sizeof(double)>>>(htrans,halpha,hbeta,hm,hn,hrowptr,hcolindex,hvalue,hx,hy, cudaRowCounter);
}