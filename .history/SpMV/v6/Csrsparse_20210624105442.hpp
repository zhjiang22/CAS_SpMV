                  ...
                  double* hy)
{
	...
	int block_dim = 1024;
	spmv_pcsr_kernel1<T><<<ceil(NonZ/(float)BlockDim),BlockDim>>>(d_val,d_vector,d_cols,mat->nz,d_v);
    spmv_pcsr_kernel2<T><<<ceil(hm/(float)threadsPerBlock),threadsPerBlock>>>(d_v,d_ptr,mat->M,d_out);
}