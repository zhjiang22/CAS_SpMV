                  ...
                  double* hy)
{
	...
	spmv_pcsr_kernel1<T><<<ceil(NonZ/(float)block_dim),block_dim>>>(d_val,d_vector,d_cols,mat->nz,d_temp);
    spmv_pcsr_kernel2<T><<<ceil(hm/(float)64),64>>>(d_v,d_ptr,mat->M,d_out);
}