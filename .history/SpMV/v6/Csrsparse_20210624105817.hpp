                  ...
                  double* hy)
{
	...
	spmv_pcsr_kernel1<T><<<ceil(NonZ/(float)block_dim),block_dim>>>(hvalue,hy,hcolindex,NonZ,d_temp);
    spmv_pcsr_kernel2<T><<<ceil(hm/(float)64),64>>>(d_temp,hrowptr,hm,d_out);
}