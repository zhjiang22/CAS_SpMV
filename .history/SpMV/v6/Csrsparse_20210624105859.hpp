...
__global__ void spmv_pcsr_kernel1(int * d_val,__SIZEOF_LONG_DOUBLE__ * d_vector,int * d_cols,int d_nnz, __SIZEOF_LONG_DOUBLE__ * d_v)
{
                  ...
                  double* hy)
{
	...
	spmv_pcsr_kernel1<T><<<ceil(NonZ/(float)block_dim),block_dim>>>(hvalue,hx,hcolindex,NonZ,d_temp);
    spmv_pcsr_kernel2<T><<<ceil(hm/(float)64),64>>>(d_temp,hrowptr,hm,hy);
}