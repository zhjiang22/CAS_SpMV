...
__global__ void device_sparse_spmv(int trans,
								   ...
								   int *cudaRowCounter)
{
	int laneId = threadIdx.x & ((1 << power) - 1) ; //lane index in the vector
	...
	extern __shared__ volatile int space[][2];
  	...
  	int row = __shfl(row, 0) + warpVectorId;
  	while (row < m) {
// Use two threads to fetch the row offset
    ...
    // Get a new row index
    	if(warpLaneId == 0) row = atomicAdd(cudaRowCounter, warpSize >> THREADS_PER_VECTOR);
...
double* hy)
{
	...
	int temp = 2; // the power of 2
	int block_dim = 256;
	int grid_dim = (hm + block_dim - 1) / (block_dim);
    device_sparse_spmv<<<grid_dim, block_dim, block_dim / temp * sizeof(double)>>>(htrans,halpha,hbeta,hm,hn,hrowptr,hcolindex,hvalue,hx,hy, 2, cudaRowCounter);
}