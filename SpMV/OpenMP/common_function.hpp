...
template<typename T>
void print_vector(vector<T> &vecTest)
{   
for (auto it : vecTest)
{
...
void OpenMP_spmv_static(int alpha,int beta,dtype *value,int *rowptr,int *colindex,int m,int n,int a,dtype *x,dtype *y){
    //calculate the matrix-vector multiply where matrix is stored in the form of CS
...
void OpenMP_spmv_guided(int alpha,int beta,dtype *value,int *rowptr,int *colindex,int m,int n,int a,dtype *x,dtype *y){
    //calculate the matrix-vector multiply where matrix is stored in the form of CS
    #pragma omp parallel for\
            num_threads(24)\
            schedule(guided)
...
void BinarySearch(int start_k, int end_k, const int* rowptr, int m, int NonZ,
...
void OpenMP_merged_spmv(int alpha,int beta,dtype *value,int *rowptr,int *colindex,int m,int n,int a,dtype *x,dtype *y){
    //calculate the matrix-vector multiply where matrix is stored in the form of CS
    int p = 24;
    int *row_carray = new int[p];
    int *value_carray = new int[p];
    int nnz = rowptr[m];
#pragma omp parallel num_threads(p)
{
    ...
    int sum_k = m + nnz;
    int items_per_thread = (sum_k + num_threads - 1) / num_threads;
    int start_k = std::min(items_per_thread * tid, sum_k);
    int end_k = std::min(start_k + items_per_thread, sum_k);

    int begin_x, begin_y, end_x, end_y;
    BinarySearch(start_k, end_k, rowptr, m, nzz, &begin_x, &begin_y, &end_x, &end_y);
	double sum;
	...
	for (; begin_y < end_y; ++begin_y)
		sum += value[begin_y] * x[colindex[begin_y]];
    row_carray[tid] = end_x;
    value_carray[tid] = sum;
}
    for (int tid = 0; tid < p - 1; tid++) {
        if (row_carray[tid] < m)
             y[row_carray[tid]] += alpha * value_carray[tid];
    }
}
